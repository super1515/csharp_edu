...
Наряду со стандартным подходом к форматированию строк, существует также алтернативный синтаксис, который называется интерполяцией строк. При таком подходе нет необходимости использовать метод ``string.Format()`` и передавать переменные в виде аргументов функции:
```csharp
string myName = "Nick";
