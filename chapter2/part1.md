...
В C# существуют 4 итерационные конструкции:
* Цикл for
* Цикл foreach
* Цикл while
* Цикл do/while
...
Цикл ``for`` позволяет производить различные манипуляции с итерационной переменной. Простейший цикл ``for`` выглядит следующим образом:
```csharp
...
Цикл ``foreach`` позволяет пройтись по всем элементам итерируемой переменной:
```csharp
string myStr = "Hello World!";
...
В качестве итерируемой переменной может выступать любой класс, реализующий интерфейс ``IEnumerable``.

Так же может быть удобно использовать ключевое слово ``var`` в цикле ``foreach``:
```csharp
...
Тело цикла ``while`` выполняется до тех пор пока истинно условие:
```csharp
string myStr = "Hello World!";
int i = 0;
...
Ключевое слово ``continue`` в контексте цикла позволяет досрочно перейти к следующей итерации:
```csharp
string myStr = "Hello World!";
foreach(char ch in myStr){
    if(ch == ' ') continue;
...
Ключевое слово ``break`` в контексте цикла позволяет досрочно закончить выполнение цикла:
```csharp
if(ch == ' ') break;
```

В первом случае будут выведены все символы строки кроме пробела, а во втором все символы до первого вхождения пробела.

## Конструкции принятия решения

Для управления потоком выполнения программы в C# существуют следующие конструкции:
* Оператор if/else if/else
* Оператор switch

### Оператор if/else if/else

Оператор if/else if/else работает исключительно с булевыми выражениями. Допустимые операции отношения и равенства в булевых выражениях:
...
Условные логические операторы отличаются от побитовых тем, что они могут быть использованы не только при работе с целыми числами. А так же условные операторы не оценивают второй операнд если первого достаточно.
...
if(num > 0){
    Console.WriteLine($"{num} greater than zero");
    if(num % 2 == 0) Console.WriteLine($"{num} multiple of two");
}else if(num < 0) {
    Console.WriteLine($"{num} less than zero");
}
else {
...
if (myObj is int myInt)
    Console.WriteLine($"{myInt} is an int");
```

C# 9.0 внес некоторые улучшения в механизм сопоставления с образцом. Теперь можно использовать ключевые слова ``and``, ``or``, ``not``:

```csharp
int num = 5;
object obj = "Hello";
if (num is < 10 and > 0)
    Console.WriteLine($"{num} is a positive number less than 10");
if (num is > 0 and (5 or 10))
    Console.WriteLine($"{num} is a positive number equal to 5 or 10");
if (num is > 0 and not (5 or 10))
    Console.WriteLine($"{num} is a positive number not equal to 5 or 10");
if (obj is not null and string)
    Console.WriteLine($"{obj} is a string");
```

### Тернарная условная операция, изменения [C# 7.2]

Тернарная условная операция является сокращением классического условного оператора if/else:
```csharp
string str = "Hello World!";
Console.WriteLine(str.Length > 0 ? "str > 0" : "str <= 0");
```

Тернарные условные операторы могут быть использованы только в операторах присвоения, при передаче в метод или возврате из него.

В C# 7.2 появилась возможность использовать тернарный оператор для получения ссылки или для изменения значения по ней:
```csharp
...
Применение оператора ``goto`` в образцах с типами больше невозможно.
